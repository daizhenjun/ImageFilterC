...
#if !defined(LabyrinthTextureH)
#define LabyrinthTextureH
...
class LabyrinthTexture : public ITextureGenerator{
private:
	int	r;
public:
    ...
    virtual vector<vector<float>> Generate( int width, int height )
	{
		...
		PerlinNoise noise( 1.0 / 16, 1.0, 0.65, 1 );
		...
		for ( int y = 0; y < height; y++ )
		{
			for ( int x = 0; x < width; x++ )
			{
				texture[y][x] = 
					min( 1.0f,
						(float) abs( noise.Function2D( x + r, y + r ) )
					);
